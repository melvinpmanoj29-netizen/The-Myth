...
export function Footer() {
  ...
  return (
    <footer className="relative bg-charcoal-950 border-t border-charcoal-800 overflow-hidden">
      ...
      <div className="relative w-full aspect-[16/7] sm:aspect-[21/8] min-h-[300px] sm:min-h-[380px] md:min-h-[440px] overflow-hidden border-b border-charcoal-800/80">
        <Image
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-end pb-8 sm:pb-12 text-center p-4 z-10 space-y-3">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-charcoal-900/90 border border-charcoal-700 text-[11px] font-mono text-myth-cyan tracking-widest uppercase shadow-md">
            <span className="w-1.5 h-1.5 rounded-full bg-myth-cyan animate-pulse" />
            RESONANCE NETWORK // ONLINE
          </div>
          <p className="font-display font-black text-xl sm:text-2xl md:text-3xl uppercase tracking-tight text-white max-w-xl leading-snug drop-shadow-[0_4px_15px_rgba(0,0,0,0.9)]">
      ...
      <div className="py-8 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto flex flex-col md:flex-row items-center justify-between gap-6">
        {/* Left Branding */}
        ...
        <nav className="flex items-center gap-5 sm:gap-8">
          {footerLinks.map((link) => (
            <a
              ...
              className="text-xs font-mono tracking-wider text-text-muted hover:text-white transition-colors"
            >
              {link.name}
            </a>
          ))}
        ...
        <div className="flex items-center gap-2 text-right">
          ...
          <span className="w-4 h-[2px] bg-myth-red-bright inline-block" />
        </div>
      </div>
    </footer>
  );
